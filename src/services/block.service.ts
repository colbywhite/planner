...
type BlockTypes = Database['public']['Tables']['blocks']
type NumberedBlockTypes = Database['public']['Views']['numbered_blocks']
type NonNullableFields<T> = {
    [P in keyof T]: NonNullable<T[P]>;
};
type Block = NonNullableFields<NumberedBlockTypes['Row']> & Pick<NumberedBlockTypes['Row'], 'subtitle'>
...
export class BlockService {
    ...
    private async safeOperation<Result>(query: PromiseLike<PostgrestSingleResponse<Result>>) {
    ...
    public async getBlockAndTasks(index: number) {
        ...
        let block: BlockWithTasks;
        const specific_block = await this.safeOperation(
            this.client.from('numbered_blocks')
                .select('*, tasks(*)')
                .eq('index', safeIndex)
        ...
        if(specific_block===null) {
            ...
            const last_block = await this.safeOperation(
                this.client.from('numbered_blocks')
                    .select('*, tasks(*)')
